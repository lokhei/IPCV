...
void hough_transform(Mat &input, Mat &mag, Mat &direction, int r_min, int r_max, double threshold);
...
int main( int argc, char** argv ) {
    ...
    hough_transform(image, thres_mag, direction, 10, 100, 20);
...
void hough_transform(Mat &image, Mat &mag,  Mat &direction, int r_min, int r_max, double threshold) {
        ...
        for (int y = 0; y < mag.cols; y++) {
            ...
            for (int r = r_min; r < r_max; r++) {
                ...
                if (hough_space[x][y][r]>threshold){
                    circle(image, Point(y,x), r, (0,0,255),1,8,0);
                }
            }
        }
    }
